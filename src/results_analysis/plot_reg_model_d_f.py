...
def plot_regression_results(y_true, y_pred, title, target='', set_name='train'):
    ...
    # Save and show
    plt.savefig(f'plots/pdf/regression_p_{set_name}.pdf', format='pdf', bbox_inches='tight')
    plt.savefig(f'plots/png/regression_p_{set_name}.png', format='png', bbox_inches='tight', dpi=150, transparent=True)
    plt.show()
