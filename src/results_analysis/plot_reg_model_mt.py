"""
Script to plot the regression model based on dataset features and model type
"""
import random
import warnings
import pandas as pd
...
def plot_regression_results(y_true, y_pred, title, target='', set_name='train'):
    ...
    # Save and show
    plt.savefig(f'plots/pdf/regression_mt_{set_name}.pdf', format='pdf', bbox_inches='tight')
    plt.savefig(f'plots/png/regression_mt_{set_name}.png', format='png', bbox_inches='tight', dpi=150, transparent=True)
    plt.show()
