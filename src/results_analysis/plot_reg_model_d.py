"""
Script to plot the regression model based on dataset features
"""
...
def plot_regression_results(y_true, y_pred, title, target='', set_name='train'):
    ...
    # Save and show
    plt.savefig(f'plots/pdf//regression_{set_name}.pdf', format='pdf', bbox_inches='tight')
    plt.savefig(f'plots/png/regression_{set_name}.png', format='png', bbox_inches='tight', dpi=150, transparent=True)
    plt.show()
...
def predict(df):
    return (
        -0.556997 *
        (0.976145 - 5.26408e-7 * df['nrinst']) *
        (
            0.000820697 * df['nsratio'] +
            1.59467 * (1 - 0.0034552 * df['nroutliers'])**2 *
            (1 / df['insttoattr'] + 0.736167)**2 *
            np.exp(
                (
                    8.36841e-5 * df['insttoattr'] +
                    0.00749363 * df['nrattr'] +
                    9.98144 * (1 - 0.21743 * df['nrclass'])**2 *
                    (df['classent'] - 0.898256)**2 *
                    (0.0582329 * df['eqnumattr'] - 0.936168) *
                    (-6.49999e-6 * df['nrinst'] - 1.90476) +
                    0.00274494
                ) /
                (-0.0142931 * df['nsratio'] - 0.0437129)
            )
            - 0.540341
        )
        + 0.570559
