...
def __init_models(global_random_seed, x_train, y_train_encoded):
    return {
...
def create_models(global_random_seed, x_train,
    ...
    models = __init_models(global_random_seed, x_train, y_train_encoded)
...
def train_best_model(model_name: str, global_random_seed: int, x_train, y_train, config: str):
    ...
    if config != 'unique':
        ...
        import json
        print(f"Json to decode {config}")
        config = json.loads(config)

        if "hidden_layer_sizes" in config:
            config["hidden_layer_sizes"]=ast.literal_eval(config["hidden_layer_sizes"])
