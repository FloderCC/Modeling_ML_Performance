...
from src.model_utils import reset_seed, train_best_model
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("log_exp_stage_ml_eval.log"),
        logging.StreamHandler()
    ]
)


# loading the results dataset to fill the model performance - cost
# df_name = f"results/results_stage_1_small_test.csv"
df_name = f"results/results_stage_1.csv"
...
for index, row in results_df_to_iterate.iterrows():
    ...
    model, num_iters = train_best_model(model_name, seed, x_train, y_train, model_parameters)
    ...
    logging.info(f"Setup {training_number}/{number_of_tunings} done: {seed},{dataset_name},{dataset_percentage},{model_name}. MCC={mcc}")
    ...
    # saving the cost metrics
    results_df.loc[(results_df['Seed'] == seed) &
                   (results_df['Dataset'] == dataset_name) &
                   (results_df['Sample Size'] == dataset_percentage) &
                   (results_df['Model'] == model_name), 'Number of Iterations'] = num_iters

    # exporting the results
    results_df.to_csv(f"results/results_stage_2.csv", index=False)
